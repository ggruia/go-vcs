...
var MsgEmptyCommitHistory = "No commits yet!"
...
var historyCmd = &cobra.Command{
	...
	RunE: func(_ *cobra.Command, _ []string) error {
		return runHistoryCommand(os.Stdout, vcsCommitDirPath)
	},
}
...
func runHistoryCommand(writer io.Writer, commitDirName string) error {
	...
	for i := dirLen; i > 0; i-- {
		...
		metadataFilePtr, _ := openFile(filepath.Join(commitDirName, vNo, vcsCommitMetadataFileName))
...
func displayHistory(writer io.Writer, history []commitHistory) {
	if len(history) == 0 {
		fmt.Println(MsgEmptyCommitHistory)
		return
	}
